...
> **Cas particulier Champagne — fiches producteur, sans niveau appellation (acté 2026-08-07).** La Champagne n'a qu'**une AOC** (Champagne) pour ~99 % du vignoble : la valeur ne vient pas de l'appellation mais du **producteur / cuvée / millésime**. On **saute donc le niveau appellation** — et **pas** de N3 « style » non plus (blanc de blancs / blanc de noirs **refusé** : ça re-liste les mêmes producteurs, contenu mince, cannibalise les fiches maison, mauvais funnel). Les fiches sont des **fiches producteur / maison** directement sous `/cotes/champagne/{maison}/` : elles occupent le **créneau d'URL N3** mais utilisent le **gabarit domaine** (§N4, détail par millésime/cuvée). Templates live : `cotes/champagne/dom-perignon/` et `cotes/champagne/cristal/`.
> - **Slug** = la cuvée-marque quand elle EST la marque (`dom-perignon`, `cristal`, `salon`), sinon la maison (`krug`, `bollinger`).
> - **Priorité** par profondeur de marché : Krug, Salon, Bollinger → Pol Roger / Taittinger / Ruinart / Philipponnat → vignerons (Selosse, Egly-Ouriet, Prévost).
> - Les **styles** (blanc de blancs, rosé, magnum) restent du **contenu explicatif dans le hub**, jamais des fiches. Les 2 seules vraies mini-AOC cotables (**Coteaux Champenois**, **Rosé des Riceys**) pourraient un jour faire une fiche N3 façon appellation, mais marché confidentiel = basse priorité.
> - **Éditorial (cadrage cliente sur Cristal 2026-08)** : **pas** de tableau de cote brut (retiré = « donne trop d'infos, n'encourage pas l'audit ») → **hiérarchie de valeur qualitative** + **graphique en valeur de revente estimée (cote ×0,8, net vendeur)** à barres flottantes sur axe € gradué + **exemple de rapport par millésime**. Graphique et rapport sur la **même base ×0,8**. Prix **toujours relevés à la source** ([[feedback_jamais_prix_non_verifie]]) : ne jamais avancer une cote non fetchée, même une « estimation de lot ».
...
6. Pour chaque entité, **relever la cote iDealwine ET le prix Wine-Searcher**, les **croiser**, préférer la **cote d'enchères iDealwine** quand elles divergent (le retail inclut la marge distributeur). **Aucun prix inventé.**
   - **Couvrir les millésimes de référence de la région** (tableau par région dans `cotes/millesimes/index.html`) : au moins une grande année d'avant 1990 et un millésime récent faible. Le **nombre** de points ne suffit pas, c'est la **couverture** qui fixe les bornes — 8 millésimes sans 1982 ont donné 12 fourchettes fausses sur 14 à Saint-Estèphe.
   - **Fiche domaine N4 — relever en plus la COURBE de cote de chaque millésime retenu**, systématiquement (série annuelle via le Browser pane + l'API interne iDealwine : voir [« La courbe de cote »](references/sourcing-et-verification.md#la-courbe-de-cote--obligatoire-avant-toute-reco-garder--vendre)). La cote ponctuelle donne le prix, la courbe donne le **cycle** — et c'est le cycle qui décide du badge garder / arbitrer / vendre. Étape **non facultative** : sans elle les recos sont fausses aussi souvent que justes (sur Ausone, 4 cartes sur 6 reprises après lecture des trajectoires).
7. Chercher un **fait d'enchères marquant** sourçable (record, adjudication notable) : maison de ventes + date + montant. À inclure plus tard dans « Tendance & liquidité ». **Étape non facultative — c'est l'inclusion qui est conditionnelle, pas la recherche** : chercher d'abord sur le web en balayant plusieurs maisons, l'API iDealwine seulement en dernier recours (voir [« Fait d'enchères marquant »](references/sourcing-et-verification.md#fait-denchères-marquant--recherche-obligatoire-inclusion-conditionnelle)). Si rien n'est sourçable, le dire à la cliente plutôt que d'omettre en silence.
