...
4. **Cœur de la page — la cote au format « cave illustrative »** (repris de l'exemple Mouton / Petrus, cf. `cotes/bordeaux/pomerol/petrus/index.html`) : plutôt qu'un tableau brut de tous les millésimes, présenter **5-6 millésimes choisis** (une légende ancienne, un ou deux grands récents, une année à l'apogée, un millésime plus modeste), chacun en **carte `.fiche`** avec un **badge garder / arbitrer / vendre** (`.badge-garder` / `.badge-arbitrer` / `.badge-vendre`) et un court commentaire ; le tout précédé d'une synthèse (`.synth-grid` : nb bouteilles, valeur totale, liquidité) et d'un bandeau **« exemple illustratif — cave fictive »** (`.report-flag`), suivi d'une **« logique d'ensemble »**. **L'« Estimation » affichée par millésime est la valeur vénale = `cote iDealwine × 0,80`, arrondie au multiple de 5 € inférieur** (branche iDealwine seule de la formule `expertise-cave` — jamais la cote brute ; cf. [`sourcing-et-verification.md`](sourcing-et-verification.md)). La synthèse « valeur estimée » = somme des estimations × quantités. Toutes les cotes relevées sur iDealwine, croisées en coulisses.
   > **Obligatoire — une courbe relevée par carte.** Le badge et le commentaire de chaque millésime se décident sur sa **trajectoire de cote**, pas sur sa cote ponctuelle : relever la série de CHAQUE millésime retenu avant d'écrire (méthode + doctrine des trois états dans [`sourcing-et-verification.md`](sourcing-et-verification.md#la-courbe-de-cote--obligatoire-avant-toute-reco-garder--vendre)). Sans la courbe, on se trompe de reco — vérifié sur Ausone, où quatre cartes sur six ont dû être reprises après lecture des trajectoires. Les données de la courbe **ne sont jamais publiées**, seulement la tendance en prose.
   > **Cohérence d'ensemble à vérifier en fin de rédaction** : deux cartes dans la même situation de marché ne peuvent pas porter des recos opposées sans que la page dise pourquoi ; et le bloc « logique d'ensemble » doit refléter les badges réellement posés (c'est en le compressant qu'on introduit des contresens).
